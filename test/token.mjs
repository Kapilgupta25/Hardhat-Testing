...
describe("Token Contract", function () {
    ...
    describe("Transactions", function () {
        ...
        it("Should update balances after transfers", async function () {
            ...
            const address1Balance = await hardhatToken.checkBalance(address1.address);
            expect(address1Balance.toNumber()).to.equal(5);
            const address2Balance = await hardhatToken.checkBalance(address2.address);
            expect(address2Balance.toNumber()).to.equal(10);
        });
    });
  });
